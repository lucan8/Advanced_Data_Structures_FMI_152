...
def main():
    ...
    test_configs_splaytree = [
        ...
        TestConfig(10_000_000, 100_000_000),
    ]

    test_generator.generate_test("SplayTree/SplayTree_insert", test_configs_splaytree)
    test_generator.generate_test("SplayTree/SplayTree_search", test_configs_splaytree)
    test_generator.generate_test("SplayTree/SplayTree_remove", test_configs_splaytree)
    ...
    splaytree_insert_path = "tests/random_generated/SplayTree/SplayTree_insert"
    splaytree_search_path = "tests/random_generated/SplayTree/SplayTree_search"
    splaytree_remove_path = "tests/random_generated/SplayTree/SplayTree_remove"

    print("Testing Splaytree...")
    for test_i, test_s, test_r in zip(
            test_handler.read_file(splaytree_insert_path),
            test_handler.read_file(splaytree_search_path),
            test_handler.read_file(splaytree_remove_path),
    ):
        print(f"Test {test_i.index}: (length={test_i.length}, max_value={test_i.max_value})")
        splaytree = SplayTree()
        splaytree.blockInsert(test_i.array)
        splaytree.blockSearch(test_s.array)
        splaytree.blockRemove(test_r.array)
